...
observe, but with the additional expressiveness come numerical complications.
Black-Scholes requires fitting only a single number known as the \emph{implied
volatility}, but the local volatility model requires fitting an entire
multi-dimensional function. We explore these complications and arrive at
...
\end{align*}
For simplicity, we assume none of these are dependent on $t$. For each local volatility function, we simulated the movement of the underlying for one year with 100 intervals per year, and calculated simulated \emph{true} option prices for each month, with \$1 apart strike prices based on $10^5$ samples. To further reduce Monte Carlo error in the true prices, we average them over 100 trials of the above procedure.
 
\begin{figure}[h]
  \centering
  \includegraphics{figs/local_vols.eps}
  \caption{Local Volatility Functions Considered in Numerical Experiments}
  \label{fig:local_vols}
\end{figure}

Then, we apply the methods discussed in Section~\ref{sec:localvolfitting} to each observed true prices. Since the true prices form a grid, we obtain a grid of estimated local volatility. We construct a function $\tilde\sigma$ over all values of $S,t$ from its observed values on a grid by taking the nearest neighbor, i.e., the $\tilde \sigma$-image of a point $S,t$ not in the grid is the closest observed local volatility estimate. This is implemented via the library function \texttt{NearestNDInterpolator} in \texttt{scipy.interpolate}. We then use the interpolated $\tilde \sigma$ to price options via Monte Carlo pricing, as in Section~\ref{sec:localvolpricing}. We then compare the predicted prices to the actual prices.
